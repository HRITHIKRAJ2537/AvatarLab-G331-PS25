...
//   const [sentences, setSentences] = useState<VoiceSentence[]>([]);
...
//   // Fetch sentences from /api/audio/select
//   useEffect(() => {
...
//         const fetchedSentences = data.audioFiles.map((file: any) => ({
//           id: file.id,
//           normalizedText: file.normalizedText,
//         }));
//         setSentences(fetchedSentences);
//         if (fetchedSentences.length > 0) {
//           setSelectedSentence(fetchedSentences[0].normalizedText);
//         } else {
//           setErrors((prev) => ({ ...prev, sentence: "No sentences available to recite." }));
//         }
//       } catch (error) {
...
//           variant: "destructive",
//           title: "Error",
//           description: "Failed to load sentences to recite. Please try again.",
//         });
//         setErrors((prev) => ({ ...prev, sentence: "Failed to load sentences. Please try again." }));
//       }
//     };
...
//             description: "Recording stopped after 30 seconds.",
...
//       const response = await fetch("/api/audio/upload-voice", {
//         method: "POST",
//         body: formData,
//       });
...
//           description: "Your voice has been uploaded successfully",
//           className: "bg-blue-50 border-blue-200",
//         });
//         // setTimeout(() => {
//         //   router.push("/voices");
...
export default function UploadVoicePage() {
  ...
  useEffect(() => {
    const fetchSentences = async () => {
      try {
        const userIdentifier = await getUserId();
        console.log("fetchSentences - userIdentifier:", userIdentifier);
        if (!userIdentifier) {
          toast({
            variant: "destructive",
            title: "Authentication Error",
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const userIdentifier = await getUserId();
      console.log("handleSubmit - userIdentifier:", userIdentifier);
      if (!userIdentifier) {
        console.log("handleSubmit - No userIdentifier, redirecting to login");
        toast({
          variant: "destructive",
          title: "Authentication Error",
          description: "You are not logged in. Please sign in first.",
